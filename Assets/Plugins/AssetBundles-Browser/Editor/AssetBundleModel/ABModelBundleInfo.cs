...
namespace AssetBundleBrowser.AssetBundleModel
{
    internal sealed class BundleTreeItem : TreeViewItem
    {
        ...
        {
            get { return AssetBundleBrowserMain.instance.m_ManageTab.hasSearch ? m_Bundle.m_Name.fullNativeName : m_Bundle.displayName; }
        }
    }
    ...
    internal class BundleNameData
    {
        ...
        internal void PartialNameChange(string newToken, int indexFromBack)
        {
            ...
            else if (indexFromBack - 1 < m_PathTokens.Count) m_PathTokens[m_PathTokens.Count - indexFromBack] = newToken;
            GenerateFullName();
        }
    ...
    internal abstract class BundleInfo
    {
        ...
        protected virtual void RefreshMessages()
        {
            ...
            MessageSystem.MessageFlag flag = m_BundleMessages.HighestMessageFlag();
        ...
        {
            if (isRootOfDelete) m_Parent.HandleChildRename(m_Name.shortName, string.Empty);
        }
    ...
    internal class BundleDataInfo : BundleInfo
    {
        ...
        ~BundleDataInfo()
        {
            foreach (AssetInfo asset in m_DependentAssets) Model.UnRegisterAsset(asset, m_Name.fullNativeName);
        }
        ...
        internal override void RefreshAssetList()
        {
            ...
            string[] assets = Model.DataSource.GetAssetPathsFromAssetBundle(m_Name.fullNativeName);
            foreach (string assetName in assets)
            {
                ...
                if (string.IsNullOrEmpty(bundleName))
                {
                    ...
                    if (!string.IsNullOrEmpty(bundleName))
                    {
                        AssetInfo folderAsset = Model.CreateAsset(partialPath, bundleName);
                        folderAsset.isFolder = true;
                        if (m_ConcreteAssets.FindIndex(a => a.displayName == folderAsset.displayName) == -1) m_ConcreteAssets.Add(folderAsset);

                        AssetInfo newAsset = Model.CreateAsset(assetName, folderAsset);
                        if (newAsset != null)
                        {
                            m_DependentAssets.Add(newAsset);
                            if (m_DependentAssets != null && m_DependentAssets.Count > 0)
                            {
                                AssetInfo last = m_DependentAssets.Last();
                                if (last != null)
                ...
                {
                    AssetInfo newAsset = Model.CreateAsset(assetName, m_Name.fullNativeName);
                    if (newAsset != null)
                    {
            ...
            if (sceneError)
                foreach (AssetInfo asset in m_ConcreteAssets)
                    if (asset.isFolder)
                    {
        ...
        internal override void AddAssetsToNode(AssetTreeItem node)
        {
            ...
            foreach (AssetInfo asset in m_DependentAssets)
        ...
        private void GatherDependencies(AssetInfo asset, string parentBundle = "")
        {
            ...
            List<AssetInfo> deps = asset.GetDependencies();
            if (deps == null)
                return;

            foreach (AssetInfo ai in deps)
            {
                if (ai == asset || m_ConcreteAssets.Contains(ai) || m_DependentAssets.Contains(ai))
                    continue;

                string bundleName = Model.DataSource.GetImplicitAssetBundleName(ai.fullAssetName);
                if (string.IsNullOrEmpty(bundleName))
                {
                    m_DependentAssets.Add(ai);
                    m_TotalSize += ai.fileSize;
                    if (Model.RegisterAsset(ai, parentBundle) > 1) SetDuplicateWarning();
                }
        ...
        internal override void HandleReparent(string parentName, BundleFolderInfo newParent = null)
        {
            ...
            foreach (AssetInfo asset in m_ConcreteAssets) Model.MoveAssetToBundle(asset, newName, m_Name.variant);
    ...
    internal class BundleVariantDataInfo : BundleDataInfo
    {
        ...
        {
            RefreshAssetList();
            if (isRootOfDelete) m_Parent.HandleChildRename(m_Name.variant, string.Empty);
            Model.MoveAssetToBundle(m_ConcreteAssets, forcedNewName, forcedNewVariant);
        }
        ...
        internal bool FindContentMismatch(BundleVariantDataInfo other)
        {
            ...
            {
                HashSet<string> myUniqueAssets = new HashSet<string>();
                HashSet<string> otherUniqueAssets = new HashSet<string>(other.m_FolderIncludeAssets.Select(x => x.displayName));

                foreach (AssetInfo asset in m_FolderIncludeAssets)
            ...
            {
                HashSet<string> myUniqueAssets = new HashSet<string>();
                HashSet<string> otherUniqueAssets = new HashSet<string>(other.m_ConcreteAssets.Select(x => x.displayName));
    ...
    internal abstract class BundleFolderInfo : BundleInfo
    {
        ...
        internal override bool HandleRename(string newName, int reverseDepth)
        {
            ...
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.HandleRename(newName, reverseDepth + 1);
            return true;
        }
        ...
        {
            base.HandleDelete(isRootOfDelete);
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.HandleDelete(false, forcedNewName, forcedNewVariant);
            m_Children.Clear();
        }
        ...
        internal override bool RefreshEmptyStatus()
        {
            ...
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) empty |= child.Value.RefreshEmptyStatus();
        ...
        internal override void RefreshAssetList()
        {
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.RefreshAssetList();
        }

        internal override bool RefreshDupeAssetWarning()
        {
            bool dupeWarning = false;
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) dupeWarning |= child.Value.RefreshDupeAssetWarning();
        ...
        internal override void AddAssetsToNode(AssetTreeItem node)
        {
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.AddAssetsToNode(node);
            m_Dirty = false;
        }
        ...
        internal override bool doneUpdating
        {
            get
            {
                foreach (KeyValuePair<string, BundleInfo> child in m_Children) m_DoneUpdating &= child.Value.doneUpdating;
        ...
        internal override List<AssetInfo> GetDependencies()
        {
            ...
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) assets.AddRange(child.Value.GetDependencies());
            return assets;
        }
    }
    ...
    internal class BundleFolderConcreteInfo : BundleFolderInfo
    {
        ...
        {
            RefreshMessages();
            BundleTreeItem result = new BundleTreeItem(this, depth, Model.GetFolderIcon());
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) result.AddChild(child.Value.CreateTreeView(depth + 1));
            return result;
        }
        ...
        internal override void HandleReparent(string parentName, BundleFolderInfo newParent = null)
        {
            ...
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.HandleReparent(newName);
    ...
    internal class BundleVariantFolderInfo : BundleFolderInfo
    {
        ...
        internal void ValidateVariants()
        {
            ...
            if (m_Children.Count > 1)
            {
                ...
                foreach (KeyValuePair<string, BundleInfo> c in m_Children)
                {
                    BundleVariantDataInfo child = c.Value as BundleVariantDataInfo;
        ...
        internal override BundleTreeItem CreateTreeView(int depth)
        {
            ...
            BundleTreeItem result = new BundleTreeItem(this, depth, icon);
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) result.AddChild(child.Value.CreateTreeView(depth + 1));
            return result;
        }
        ...
        internal override void HandleReparent(string parentName, BundleFolderInfo newParent = null)
        {
            ...
            foreach (KeyValuePair<string, BundleInfo> child in m_Children) child.Value.HandleReparent(parentName);
